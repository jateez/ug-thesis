...
const RAW_QUERY_URL = BASE_URL + "/blogs/raw-query";
...
export function readRAW() {
  ...
  const res1 = http.get(RAW_QUERY_URL + `?q=${pageNum}`);
  ...
  const res2 = http.get(RAW_QUERY_URL + `/${articleId}`);
  check(res2, {
    "is status 200": (r) => r.status === 200,
...
export function handleSummary(data) {
  ...
  return {
    "summary-read-raw.json": JSON.stringify(data, null, 2),
